...
function onTokenDelete(tokenMap)
	...
	if nodeCT then
		...
		DB.setValue(nodeCT, "tokenonmap", "number", 0);
	end
end
...
function getTurnIcon(nodeEntry)
	local actor = ActorManager.resolveActor(nodeEntry);
	if actor then
...
function centerOnToken(nodeEntry)
	local tokeninstance = getTokenFromCT(nodeEntry);
	if tokeninstance then
		local nodeImage = tokeninstance.getContainerNode();
		local w = Interface.findWindow("imagewindow", nodeImage.getParent());
		if w then
			local x,y = tokeninstance.getPosition();
			w.image.setViewpointCenter(x,y);
		end
